...
from app.config import (
    DB_CONN_PARAMS,  # Usar los parámetros de conexión generales
    LOG_WRITER_DB_ENABLED,  # Basado en si DB_CONN_PARAMS está completo
...
class LogWriterDB:
    ...
    def __new__(cls, *args, **kwargs) -> Optional["LogWriterDB"]:
        if not LOG_WRITER_DB_ENABLED:  # Usar la variable de config actualizada
        ...
        with cls._lock:
            if cls._instance is None:
                ...
                # Attempt to create connection pool if configured in DB_CONN_PARAMS
                if DB_CONN_PARAMS.get("pool_name"):
                    try:
                        # mysql.connector.pooling.MySQLConnectionPool no debe tener argumentos desconocidos
                        pool_creation_params = {
                            "pool_name": DB_CONN_PARAMS["pool_name"],
                            "pool_size": DB_CONN_PARAMS.get(
                                "pool_size", 5
                            ),  # Default pool_size
                            ...
                            # Añadir otros parámetros de conexión válidos si los tienes (charset, etc.)
                        }
    ...
    def __init__(self):
        if not LOG_WRITER_DB_ENABLED:  # Chequeo de nuevo por si acaso
            return
    ...
    def _get_connection(self) -> Optional[mysql.connector.MySQLConnection]:
        try:
            if self._db_pool:  # type: ignore
                ...
                # Crear conexión individual usando DB_CONN_PARAMS, quitando los de pool si existen
                conn_params_no_pool = {
                    k: v
    ...
    # _start_worker, add_log_entry_data, _process_queue_loop, _insert_batch_to_db
    # permanecen iguales en su lógica interna, ya que _get_connection
    # ahora usa los parámetros de la única BD.

    def _start_worker(self):  # Sin cambios lógicos
    ...
    def add_log_entry_data(
        ...
        try:
            ...
            logger.error(
                f"LogWriterDB: Error adding log entry to queue: {e}", exc_info=True
            )

    def _process_queue_loop(
        self,
    ):  # Sin cambios lógicos, solo la forma en que _get_connection funciona
        logger.info("LogWriterDB processing loop started.")
        last_flush_time = time.time()

        while not self.stop_event.is_set() or not self.log_queue.empty():  # type: ignore
            ...
            try:
                while len(batch) < LOG_WRITER_BATCH_SIZE:
                    ...
                    if self.stop_event.is_set() and self.log_queue.empty():  # type: ignore
                        break

                    try:
                        item = self.log_queue.get(block=True, timeout=get_timeout)  # type: ignore
                        batch.append(item)
                        self.log_queue.task_done()  # type: ignore
                    except Empty:
                        break
                ...
                if batch:
                    ...
                    last_flush_time = time.time()
                elif current_time - last_flush_time > LOG_WRITER_FLUSH_INTERVAL_SECONDS and not self.log_queue.empty():  # type: ignore
                    pass

            except Exception as e:
                logger.error(
                    f"LogWriterDB: Unexpected error in processing loop: {e}",
                    exc_info=True,
                )
                time.sleep(1)
            ...
            if self.stop_event.is_set() and self.log_queue.empty():  # type: ignore
                logger.debug(
                    "LogWriterDB: Stop event set and queue empty, exiting processing loop."
                )
                break

        logger.info("LogWriterDB processing loop finished.")

    def _insert_batch_to_db(self, batch: List[LogEntryData]):  # Sin cambios lógicos
        if not batch:
            return
        ...
        try:
            ...
            if not conn:
                logger.error(
                    "LogWriterDB: Cannot insert batch, no DB connection available. Batch lost."
                )
                return
            ...
            logger.error(
                f"LogWriterDB: Error inserting log batch: {err}. Batch may be lost."
            )
            if conn:
                try:
                    conn.rollback()
            ...
            logger.error(
                f"LogWriterDB: Unexpected error inserting log batch: {e}. Batch may be lost.",
                exc_info=True,
            )
            if conn:
                try:
    ...
    def shutdown(self):  # Sin cambios lógicos
        ...
        try:
            self.log_queue.put_nowait(("", "", "", "", "", "", "SHUTDOWN_FLUSH"))  # type: ignore
        except Full:
            logger.debug(
